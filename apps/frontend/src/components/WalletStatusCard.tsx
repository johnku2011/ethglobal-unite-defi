...
export default function WalletStatusCard({ className = '' }: WalletStatusCardProps) {
  const { 
    connectedWallets, 
    isEthereumConnected, 
    isSuiConnected,
    ...
    <div className={`bg-white rounded-lg border border-gray-200 p-6 ${className}`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="font-medium text-gray-900">
                      {wallet.provider}
                      {wallet.type === 'ethereum' && ` (${getChainName(wallet.chainId)})`}
      ...
      <div className="mt-4 pt-4 border-t border-gray-200">
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${isEthereumConnected ? 'bg-green-500' : 'bg-gray-300'}`} />
            <span className={isEthereumConnected ? 'text-green-600' : 'text-gray-500'}>
              Ethereum {isEthereumConnected ? 'Connected' : 'Disconnected'}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${isSuiConnected ? 'bg-green-500' : 'bg-gray-300'}`} />
            <span className={isSuiConnected ? 'text-green-600' : 'text-gray-500'}>
