  ...
  if (!ready || !authenticated) {
    return (
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 m-4">
        <h3 className="font-semibold text-yellow-800 mb-2">🔍 Wallet Debug Info</h3>
    ...
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 m-4">
      ...
      <div className="mb-4">
        ...
        <div className="text-sm text-blue-600 space-y-1 font-mono">
          ...
          <div>Chain ID (decimal): {user?.wallet?.chainId ? parseInt(user.wallet.chainId, 16) : 'None'}</div>
      ...
      {/* Connected Wallets */}
      <div className="mb-4">
        <h4 className="font-medium text-blue-700 mb-2">Connected Wallets ({connectedWallets.length}):</h4>
            ...
            {connectedWallets.map((wallet, index) => (
              <div key={index} className="bg-blue-100 rounded p-2 text-sm">
                <div className="font-mono">
                  ...
                  <div><strong>Type:</strong> {wallet.type}</div>
                  <div><strong>Provider:</strong> {wallet.provider}</div>
                  <div><strong>Chain ID:</strong> {wallet.chainId || 'undefined'}</div>
                  <div><strong>Network:</strong> {wallet.network || 'undefined'}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="mb-4">
        ...
        <div id="chain-check-result" className="text-sm text-blue-600 mt-2"></div>
      </div>

      {/* Raw User Object */}
      <details className="mt-4">
        <summary className="font-medium text-blue-700 cursor-pointer">Raw User Object (Click to expand)</summary>
