...
const waitForWallets = (timeout = 5000): Promise<void> => {
  return new Promise((resolve) => {
    ...
    const checkWallets = () => {
      ...
      if (standardWallets.length > 0 || hasLegacyWallets || attempts >= maxAttempts) {
...
export const debugWalletDetection = async (): Promise<void> => {
  ...
  standardWallets.forEach((wallet, index) => {
    ...
    console.log(`     - Chains: ${wallet.chains.map(c => c.split(':')[1]).join(', ')}`)
  ...
  tests.forEach(test => {
    ...
    if (test.exists) {
      ...
      console.log(`  - Has connect method:`, typeof obj?.connect === 'function')
...
export const isSlushWalletInstalled = async (): Promise<boolean> => {
  ...
  // Check Wallet Standard first
  const standardWallets = getWallets().get()
  const slushWallet = standardWallets.find(wallet => 
...
export const connectSlushWallet = async (): Promise<{ address: string } | null> => {
  ...
  try {
    ...
    const wallet = (window as any).suiWallet || (window as any).sui || (window as any).slush || (window as any).SuiWallet
